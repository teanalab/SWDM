...
class TestEmbeddingSpace(unittest.TestCase):
    ...
    def test_find_unigrams_in_embedding_space(self):
        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 1, "lower_threshold": 0, "n_max": 5}
        unigrams = [(i, [(i, 1)]) for i in "hello world how are you".split(' ')]
        unigrams = list(self.embedding_space.find_unigrams(unigrams))
        print(unigrams, file=sys.stderr)
        unigrams_expected = [('hello', []), ('world', []),
                             ('how', [('much', 0.438563734292984), ('pretty', 0.4094923734664917)]),
                             ('are', [('ones', 0.4666799008846283), ('must', 0.3789885640144348)]),
                             ('you', [('youre', 0.6245741844177246)])]
        self.assertEqual(unigrams, unigrams_expected)

    def test_find_unigrams_in_embedding_space_1(self):
        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 0.5, "lower_threshold": 0.4,
                                                              "n_max": 5}
        unigrams = [(i, [(i, 1)]) for i in "hello world how are you".split(' ')]
        unigrams = list(self.embedding_space.find_unigrams(unigrams))
        print(unigrams, file=sys.stderr)
        unigrams_expected = [('hello', []), ('world', []),
                             ('how', [('much', 0.438563734292984), ('pretty', 0.4094923734664917)]),
                             ('are', [('ones', 0.4666799008846283)]), ('you', [])]
        self.assertEqual(unigrams, unigrams_expected)
    ...
    def test_check_if_already_stem_exists(self):
        ...
        unigram = "Where"
        orig_unigram = "You"
        res = self.embedding_space.check_if_already_stem_exists(unigrams_in_embedding_space_pruned, unigram,
                                                                orig_unigram)
    ...
    def test_gen_similar_words(self):
        t = time.process_time()
        res = self.embedding_space._gen_similar_words('hello', self.embedding_space.word2vec)
    ...
    def test_check_if_unigram_should_be_added(self):
        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 0.8, "lower_threshold": 0.6,
                                                              "n_max": 5}
        res = self.embedding_space.check_if_unigram_should_be_added("planemaker", 0.75, [], "Airbus")
        self.assertFalse(res)

        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 0.8, "lower_threshold": 0.6,
                                                              "n_max": 5}
        res = self.embedding_space.check_if_unigram_should_be_added("civil", 0.6548983454704285, [], "yours")
        self.assertTrue(res)

        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 0.8, "lower_threshold": 0.7,
                                                              "n_max": 5}
        res = self.embedding_space.check_if_unigram_should_be_added("civil", 0.6548983454704285, [], "yours")
        self.assertFalse(res)

        self.embedding_space.parameters.params["word2vec"] = {"upper_threshold": 0.6, "lower_threshold": 0.5,
                                                              "n_max": 5}
        res = self.embedding_space.check_if_unigram_should_be_added("civil", 0.6548983454704285, [], "yours")
        self.assertFalse(res)
